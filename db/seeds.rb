...
template6 = Atemplate.create(title: "{{We Messed Up}}", 
  content: "<div>Hey {{FIRST NAME}},<br><br></div><div>You have every right to be upset. I sincerely apologize for the issues we had with the website yesterday. We clearly didn't have the necessary infrastructure in place to handle {{SPECIFIC ERROR}}.Anytime something like this happens, we learn a lot about how {{PRODUCT NAME}} can get better.<br><br></div><div>Yesterday was no exception and we are already hard at work on changes to prevent this from happening again. Finally, we have {{credited your account $25, which will be subtracted from your next bill}}. Hopefully this lightens the blow a little and shows how serious we are about keeping {{PRODUCT NAME}} up and running.<br><br></div><div>My personal phone number is {{XXX-XXX-XXXX}}. <br><br></div><div>If you have any further questions or feedback, it would be my pleasure to listen. Thanks very much for your time.<br><br></div><div>Sincerely,</div><div>{{Name}}</div><div>{{Title}}</div>", 
